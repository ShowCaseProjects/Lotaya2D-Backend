  ...
  async registerUserAccountOtpConfirm(
    ...
    try {
      ...
      const twoMinutesAgo = new Date(
        dayjs(userAccount.sms_send_time).format('YYYY-MM-DD HH:mm:ss'),
      ) as any;
      const diffTime = (currentTime - twoMinutesAgo) / (60 * 1000);
      if (
        userAccount.otp_code === otpCodeConfirmReqBodyDto.otpCode.toString() &&
        Number(diffTime) <= 2
      ) {
        ...
        const payload = {
          phoneNumber: otpCodeConfirmReqPathDto.phoneNumber,
          sub: confirmOtpCode.otp_code,
        };
  ...
  async registerUserAccountPasswordConfirm(
    ...
    try {
      if (
        ...
        passwordConfirmReqBodyDto.confirmPassword
      ) {
        throw new HttpException(
          {
            errorCode: 'E1113',
