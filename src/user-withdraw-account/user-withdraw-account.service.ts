...
export class UserWithdrawAccountService {
    ...
    try {
      const registerData = await this.prisma.userWithdrawAccount.create({
        data: {
          ...
          user: {
            connect: {
              phone_number: addWithdrawAccountReqBody.phoneNumber,
            },
          },
  ...
  async updateUserWithdrawAccount(
    addWithdrawAccountReqPath: UserWithdrawAccountUpdateReqPathDto,
    addWithdrawAccountReqBody: UserWithdrawAccountUpdateReqBodyDto,
    ...
    try {
      const paymentdata = await this.prisma.userWithdrawAccount.findMany({
        ...
        where: {
          ...
          account_type: {
            contains: findAllUserWithdrawAccount.accountType,
            mode: 'insensitive',
          },
          account_name: {
            contains: findAllUserWithdrawAccount.accountName,
            mode: 'insensitive',
          },
          delete_status: 0,
      ...
      return paymentdata.map((a) => {
        ...
        (withdrawdatadto.withdrawAccountId = a.user_withdraw_account_id),
          ...
          (withdrawdatadto.registerDate = dayjs(a.register_date).format(
            'YYYY-MM-DD HH:mm:ss',
          )),
          (withdrawdatadto.updatedDate = dayjs(a.updated_date).format(
            'YYYY-MM-DD HH:mm:ss',
          ));
